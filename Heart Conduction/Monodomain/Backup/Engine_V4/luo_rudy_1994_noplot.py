"""
Luo-Rudy 1994 Dynamic (LRd) Ionic Model - No Plotting Version
=============================================================
...
# =============================================================================
# Maximal Conductances and Permeabilities
# =============================================================================
G_Na = 16.0       # Fast Na+ [mS/cm^2]
G_Ca_L = 0.09     # L-type Ca2+ [mS/cm^2] - can be up to 0.12
P_Ca = 5.4e-4     # Ca2+ permeability [cm/s]
P_Na = 6.75e-7    # Na+ permeability through L-type [cm/s]
P_K = 1.93e-7     # K+ permeability through L-type [cm/s]
...
Ca_jsr_thresh = 1.5  # Threshold for CICR [mM] - approximate
...
# =============================================================================
# Initial Conditions (from steady state at rest)
# =============================================================================
V_init = -84.0      # mV
m_init = 0.0008     # Na activation
h_init = 0.993      # Na fast inactivation
j_init = 0.995      # Na slow inactivation
d_init = 0.0        # Ca activation
f_init = 1.0        # Ca voltage inactivation
f_Ca_init = 1.0     # Ca calcium inactivation
X_init = 0.0        # K activation (IK)
Na_i_init = 10.0    # mM
K_i_init = 145.0    # mM
Ca_i_init = 0.0001  # mM (100 nM)
Ca_jsr_init = 1.8   # mM
Ca_nsr_init = 1.8   # mM
...
def beta_m(V):
    """Na activation gate beta."""
    return 0.08 * safe_exp(-V / 11.0)


def alpha_h(V):
    """Na fast inactivation gate alpha."""
    if V >= -40.0:
        return 0.0
...
def beta_h(V):
    """Na fast inactivation gate beta."""
    if V >= -40.0:
...
def alpha_j(V):
    """Na slow inactivation gate alpha."""
    if V >= -40.0:
        return 0.0
...
def beta_j(V):
    """Na slow inactivation gate beta."""
    if V >= -40.0:
...
def d_inf(V):
    """L-type Ca activation steady state."""
    return 1.0 / (1.0 + safe_exp(-(V + 10.0) / 6.24))


def tau_d(V):
    """L-type Ca activation time constant [ms]."""
    dV = V + 10.0
    if abs(dV) < 1e-7:
...
def f_inf(V):
    """L-type Ca voltage inactivation steady state."""
    return 1.0 / (1.0 + safe_exp((V + 35.06) / 8.6)) + 0.6 / (1.0 + safe_exp((50.0 - V) / 20.0))


def tau_f(V):
    """L-type Ca voltage inactivation time constant [ms]."""
    return 1.0 / (0.0197 * safe_exp(-(0.0337 * (V + 10.0))**2) + 0.02)


def f_Ca_inf(Ca_i):
    """L-type Ca calcium-dependent inactivation steady state."""
    return 1.0 / (1.0 + (Ca_i / 0.00035)**1)


def alpha_X(V):
    """K activation (IK) alpha."""
    dV = V + 30.0
    if abs(dV) < 1e-7:
        return 0.25
...
def I_Na(V, m, h, j, Na_i):
    """Fast sodium current [uA/cm^2]."""
    E_Na = calc_E_Na(Na_i)
    return G_Na * m**3 * h * j * (V - E_Na)


def I_Ca_L(V, d, f, f_Ca, Ca_i):
    """L-type calcium current [uA/cm^2].

    Using simplified GHK formulation.
    """
    # Simplified formulation (as in many implementations)
    E_Ca = calc_E_Ca(Ca_i)
    # Scale factor for calcium
    return G_Ca_L * d * f * f_Ca * (V - E_Ca)


def I_K(V, X, K_i):
    """Time-dependent potassium current [uA/cm^2]."""
    E_K = calc_E_K(K_i)
    # K_o dependence
...
def I_K1(V, K_i):
    """Time-independent potassium current [uA/cm^2]."""
    E_K = calc_E_K(K_i)
    # K_o dependence
...
def I_Kp(V, K_i):
    """Plateau potassium current [uA/cm^2]."""
    E_K = calc_E_K(K_i)
...
def I_NaCa(V, Na_i, Ca_i):
    """Sodium-calcium exchanger current [uA/cm^2]."""
...
def I_NaK(V, Na_i):
    """Sodium-potassium pump current [uA/cm^2]."""
...
def I_pCa(Ca_i):
    """Sarcolemmal calcium pump current [uA/cm^2]."""
    return I_pCa_bar * Ca_i / (K_m_pCa + Ca_i)


def I_Na_b(V, Na_i):
    """Background sodium current [uA/cm^2]."""
    E_Na = calc_E_Na(Na_i)
    return G_Na_b * (V - E_Na)


def I_Ca_b(V, Ca_i):
    """Background calcium current [uA/cm^2]."""
    E_Ca = calc_E_Ca(Ca_i)
...
def I_rel(Ca_jsr, Ca_i, d):
    """Calcium release from JSR [mM/ms].

    Simplified CICR triggered by Ca entry through L-type channels.
    """
    # Graded release proportional to L-type current and JSR load
    g_rel = K_rel * d  # Release activated by d gate
    return g_rel * (Ca_jsr - Ca_i)


def I_up(Ca_i):
    """SR calcium uptake [mM/ms]."""
    return I_up_bar * Ca_i / (Ca_i + K_up)


def I_tr(Ca_nsr, Ca_jsr):
    """Transfer from NSR to JSR [mM/ms]."""
...
def beta_Ca_i(Ca_i):
    """Myoplasmic calcium buffering factor."""
...
class LuoRudy1994:
    ...
    def __init__(self, dt=0.005):
        ...
        ----------
        dt : float
            Time step [ms]. Recommend 0.005-0.01 ms for stability.
        """
        self.dt = dt
        self.C_m = C_m

    def initialize_state(self) -> Dict[str, float]:
        """Initialize state variables to resting values."""
        return {
            'V': V_init,
            'm': m_init,
            'h': h_init,
    ...
    def step(self, state: Dict[str, float], I_stim: float = 0.0) -> Dict[str, float]:
        ...
        # Update gating variables (Rush-Larsen for HH gates)
        # m gate
        ...
        # Add stimulus current to K (assuming K+ is the charge carrier)
        dK_i = -(I_K_tot + I_stim) * A_cap / (V_myo * F)
        K_i_new = K_i + dt * dK_i

        # Ca_i (with SR and buffering)
        ...
        Ca_i_new = max(1e-8, Ca_i_new)  # Prevent negative
    ...
    def run_simulation(self, t_end: float, stim_times: list = None,
        ...
        stim_amplitude : float
            Stimulus amplitude [uA/cm^2] (negative = depolarizing)
        ...
        results = {
            ...
            'I_Ca_L': np.zeros(n_steps),
            'I_K': np.zeros(n_steps),
            'I_K1': np.zeros(n_steps),
            'I_NaCa': np.zeros(n_steps),
