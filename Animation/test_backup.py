...
# ==================================================
# SCENE 5 — HELICITY DENSITY VISUALIZATION
# Combines circle/curl visualization with helical flow
# ==================================================

class Scene5(ThreeDScene):
    """
    Simplified helicity density visualization combining:
    - Circle with curl vector (from Scene1)
    - Parallel bulk flow vector
    - Orange tangential arrows on circle
    - Helicity density equation
    - Helical particle flow (from Scene2)
    """
    def construct(self):
        self.camera.background_color = "#1a1a1a"
        
        # Camera setup - 3D view to see both vectors and flow
        self.set_camera_orientation(
            phi=75 * DEGREES,
            theta=-40 * DEGREES,
            zoom=0.6
        ...
        # ===== STEP 1: Circle visualization at origin =====
        circle = Circle(radius=1.2, color=WHITE, stroke_width=2)
        circle.shift(LEFT * 3)  # Position to left of scene
        
        self.play(Create(circle), run_time=1)
        self.wait(0.5)
        
        # ===== STEP 2: Add orange tangential arrows on circle =====
        # These represent local rotation (just like Scene1)
        ...
        # ===== STEP 3: Add curl vector (perpendicular to circle plane) =====
        curl_vector = Arrow(
            start=circle_center,
        ...
        # ===== STEP 4: Add bulk flow vector (parallel to curl) =====
        bulk_vector = Arrow(
            start=circle_center,
            end=circle_center + RIGHT * 2,
            color=C_BULK,
        ...
        # ===== STEP 5: Display helicity density equation at bottom =====
        equation = MathTex(
        ...
        # ===== STEP 6: Fade out static visualization, prepare for flow =====
        self.play(
            FadeOut(circle),
            FadeOut(tangent_arrows),
            FadeOut(curl_vector),
            FadeOut(curl_label),
            FadeOut(bulk_vector),
            FadeOut(bulk_label),
        ...
        # ===== STEP 7: Add helical flow (from Scene2) =====
        # Helical vector field parameters (same as Scene2)
        ...
        # ===== STEP 8: Dynamic particle spawning (from Scene2) =====
        np.random.seed(123)
        active_particles = []
